...
В AppConfig теперь присутствует базовая настройка hibernate, берущая данные из файла db.properties. Обратите внимание, 
...
1. В сущностях обязательно переопределяем equals и hashcode - **_DONE_**
2. import javax.persistence.*;Отключи вайлдкард в импортах ко всему проекту и исправь импорты - **_DONE_**
3. У тебя реализовано сохранение юзера в котором сразу создаётся машина.
   Сделаем немного по другому, добавив гибкости проекту:
   Создаёшь список юзеров и сохраняешь его
   Создаёшь список машин и сохраняешь его
   Вытаскиваешь/получаешь из бд список юзеров и список машин.
   Раздаёшь юзерам машины из полученного списка и сохраняешь список юзеров в бд